...
import { filter, now } from '../state/store'
    ...
    <section class="section">
      ...
      {empty ? (
        ...
        <div class="rows">
          ...
          {running.map((group) => (
            <RunGroup key={group.run.id} group={group} kind="running" defaultOpen={true} />
          ))}
          ...
          {queued.map((group, i) => (
            <RunGroup key={group.run.id} group={group} kind="queued" defaultOpen={i === 0} />
          ))}
        </div>
      )}
    </section>
